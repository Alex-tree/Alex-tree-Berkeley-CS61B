...
import src.lab7_treemap.BSTMap;
...
public class BST<Key extends Comparable<Key>> {
    ...
    public static double AverageDepth(BST n){
        int t=0;
        int count=0;
        count=SumDepth(n.root,t,count);
        return (double)count/(double)n.root.size;
    }

    private static int SumDepth(BST.Node root, int t,int count){
        if (root==null){
            return count;
        }
        count+=t;// everytime add its depth
        t++;//if root not null depth add 1
        count=SumDepth(root.left,t,count);// assign count to the new value,
        // otherwise it will equal to the value in the below statement(count+=t;).
        count=SumDepth(root.right,t,count);
        return count;
    }

    public static int lIPL(BST n){
        int count=0;
        int t=0;
        return SumDepth(n.root,t,count);
    }
